...
namespace Fidely.Framework.Compilation.Objects.Operators
{
    ...
    public class Equal<T> : BaseBuiltInComparativeOperator<T>
    {
        ...
        public Equal(string symbol) : this(symbol, false)
        {
        }
        ...
        public Equal(string symbol, bool ignoreCase) : this(symbol, ignoreCase, OperatorIndependency.Strong, null)
        {
        }
        ...
        public Equal(string symbol, bool ignoreCase, OperatorIndependency independency, string description) : base(symbol, independency, description)
        {
        ...
        {
            return new Equal<T>(Symbol, _ignoreCase, Independency, Description);
        }
        ...
        protected internal override Expression Compare(Operand left, Operand right)
        {
            ...
            if (operands.Are(typeof(decimal)) || operands.Are(typeof(Guid)) || operands.Are(typeof(DateTime)) || operands.Are(typeof(DateTimeOffset)) || operands.Are(typeof(TimeSpan)))
            {
                result = Expression.Equal(left.Expression, right.Expression);
            }
            else
            {
