...
namespace Fidely.Framework.Compilation.Objects.Operators
{
    ...
    public class Multiply : BaseBuiltInCalculatingOperator
    {
        ...
        public Multiply(string symbol, int priority)
        ...
        public override Operand Calculate(Operand left, Operand right)
        {
            ...
            {
                Warn("'{0}' doesn't support to multiply '{1}' and '{2}'.", GetType().FullName, left.OperandType.FullName, right.OperandType.FullName);
        ...
        {
            return new Multiply(Symbol, Priority, Independency, Description);
        }
    }
}
